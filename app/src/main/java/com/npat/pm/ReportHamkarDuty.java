...
public class ReportHamkarDuty extends AppCompatActivity {

    ListView lstReportHamkarDuty;
    ListAdapter adapter;
    DatabaseHelper dbh;
    SQLiteDatabase db;
    String Mobile;
    ...
    private void FillData()
    {
        ...
        db = dbh.getReadableDatabase();
         cursors = db.rawQuery("select * from OtherWorkStatusReport  Order by Code ASC", null);
        if(cursors.getCount() > 0)
        {
            ...
            adapter = new AdapterOtherWorkStatusReport(this, DataList);
            lstReportHamkarDuty.setAdapter(adapter);
        }
        cursors.close();
        db.close();
				dbh.close();
